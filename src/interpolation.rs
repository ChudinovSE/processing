...
fn calc_step (first: &TestInfo, second: &TestInfo) -> (Volume, u32) {
	let (first_day, first_month, first_year) = first.date.get_date();
	let (second_day, second_month, second_year) = second.date.get_date();

	let d_day: u32 = 
			(second_day as u32 - first_day as u32) +
			(second_month as u32 - first_month as u32)*31 +
			(second_year - first_year)*31*12;
